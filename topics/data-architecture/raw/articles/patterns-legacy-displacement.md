# patterns-legacy-displacement

Source: https://martinfowler.com/articles/patterns-legacy-displacement/
...
*   [Top](#top)
*   [The Legacy Replacement Treadmill](#TheLegacyReplacementTreadmill)
*   [Breaking the cycle](#BreakingTheCycle)
    *   [Understand the outcomes you want to achieve](#UnderstandTheOutcomesYouWantToAchieve)
        *   [Reducing the cost of change](#ReducingTheCostOfChange)
        *   [Improving the business process](#ImprovingTheBusinessProcess)
        *   [Retire an old system](#RetireAnOldSystem)
        *   [Imminent Disruption](#ImminentDisruption)
        *   [Newer technology](#NewerTechnology)
    *   [Decide how to break the problem into smaller parts](#DecideHowToBreakTheProblemIntoSmallerParts)
        *   [Getting Started](#GettingStarted)
    *   [Successfully deliver the parts](#SuccessfullyDeliverTheParts)
    *   [Change the organization to allow this to happen on an ongoing basis](#ChangeTheOrganizationToAllowThisToHappenOnAnOngoingBasis)
*   [An example: Integration Middleware Removal](#AnExampleIntegrationMiddlewareRemoval)
    *   [Understanding the outcomes](#UnderstandingTheOutcomes)
    *   [Breaking the problem up: the first seam and a refactoring](#BreakingTheProblemUpTheFirstSeamAndARefactoring)
    *   [Successfully deliver the parts: building out the functionality, maintaining the contract](#SuccessfullyDeliverThePartsBuildingOutTheFunctionalityMaintainingTheContract)
    *   [Successfully deliver the parts (cont.): transitioning into live service - using a second seam](#SuccessfullyDeliverThePartscont.TransitioningIntoLiveService-UsingASecondSeam)
    *   [Changing the organisation to allow this to happen on an ongoing basis](#ChangingTheOrganisationToAllowThisToHappenOnAnOngoingBasis)
    *   [Closing thoughts](#ClosingThoughts)

### Sidebars

*   [We want to be like Netflix](#WeWantToBeLikeNetflix)
*   [Event Storming - The Swiss Army Knife of modern process mapping](#EventStorming-TheSwissArmyKnifeOfModernProcessMapping)
*   [The hardest shift is the Paradigm Shift](#TheHardestShiftIsTheParadigmShift)

# Patterns of Legacy Displacement

Effective modernization of legacy software systems

_When faced with the need to replace existing software systems, organizations often fall into a cycle of half-completed technology replacements. Our experiences have taught us a series of patterns that allow us to break this cycle, relying on: a deliberate recognition of the desired outcomes of displacing the legacy software, breaking this displacement in parts, incrementally delivering these parts, and changing the culture of the organization to recognize that change is the unvarying reality._

05 March 2024

* * *

[![Photo of Ian Cartwright](ian.jpg)](https://www.linkedin.com/in/ian-cartwright-282952/ )

[Ian Cartwright](https://www.linkedin.com/in/ian-cartwright-282952/ )
...
[![Photo of Rob Horn](rob-250x250.jpg)](https://www.linkedin.com/in/rob-horn)
...
[![Photo of James Lewis](jlewis.jpg)](https://bovon.org/)

[James Lewis](https://bovon.org/)
...
*   [Critical Aggregator](critical-aggregator.html)
*   [Divert the Flow](divert-the-flow.html)
*   [Event Interception](event-interception.html)
*   [Extract Product Lines](extract-product-lines.html)
*   [Feature Parity](feature-parity.html)
*   [Legacy Mimic](legacy-mimic.html)
*   [Revert to Source](revert-to-source.html)
*   [Transitional Architecture](transitional-architecture.html)
...
If you'd like to hear an audio introduction to this series, we have recorded a couple of episodes of the Thoughtworks Technology Podcast. [Part](https://www.thoughtworks.com/insights/podcasts/technology-podcasts/patterns-legacy-displacement-pt1) 1 introduces the series and the clusters of patterns around understanding and breaking up the legacy. [Part 2](https://www.thoughtworks.com/insights/podcasts/technology-podcasts/patterns-of-legacy-displacement-pt2) then digs into patterns for delivering the displacement.
...
Broadly speaking this involves finding the right [seams](/bliki/LegacySeam.html) in the current business and technical architecture. Importantly, you have to consider how elements of the current solution map to different business capabilities. For legacy systems this usually means discovering how one large technical solution meets multiple business needs and then seeing if it is possible to extract individual needs for independent delivery using a new solution. Ideally these should be deliverable with minimal dependencies on each other.
...
[Create Town Plan](create-town-plan.html) †

Identify stable parts of the organisation to structure teams and software around

[Event Storming](event-storming.html) †

Technique used to understand business processes

[Identify Business Capabilities](identify-business-capabilities.html) †

Identify stable parts of the organisation to structure teams and software around

[Value Stream Map](value-stream-map.html) †

Artefact that describes how users accomplish their work
...
[Extract Value Streams](extract-value-streams.html) †

Identify and separate key value streams
...
[Canary Release](canary-release.html) †
...
[Dark Launching](dark-launching.html) †
...
[Stop the World cutover](stop-the-world.html) †
...
We need organization structure and process changes to take full advantage of the better technology, and by [Conway's Law](/bliki/ConwaysLaw.html) we also need an architecture for our technology that facilitates this. If teams and their communications are organized around the existing legacy solution and processes we may need to reorganize them using the [Inverse Conway Maneuver](/bliki/ConwaysLaw.html#icm) to match the new solution and it's architecture.
...
[Build as you mean to continue](build-as-you-mean-to-continue.html) †

Create your legacy replacement in the way you need to continue once it is live.

[Incremental Displacement](incremental-displacement.html) †

Create your legacy replacement in the way you need to continue once it is live.

[New Co](new-co.html) †

Form a brand new company to pursue a market disruption

[Protected Pilot](protected-pilot.html) †

Create a pilot program for new work and detach it from the normal corporate governance process
...
Why? To reduce existing (and increasing) license and support costs. Additionally, to mitigate the risk to the business created by operating critical functions on aged out of support middleware and database technologies.
...
During [Inception](https://martinfowler.com/articles/lean-inception/) the team ran a workshop with people who had deep knowledge of the legacy system, to collaboratively visualise both the as-is and to-be software architectures. Having done this, they found a technical seam that could be exploited in the form of messaging based integration between the legacy backend and the Integration Middleware. The Legacy backend, an aging J2EE application, placed “publish product” messages onto a queue provided by a very old version of SwiftMQ. The [Event Interception](event-interception.html) pattern would be useful here, and if implemented as a [Content-Based Router](http://www.enterpriseintegrationpatterns.com/ContentBasedRouter.html) would allow control over how messages from the legacy backend were routed, and create an option enabling messages to be routed to new systems.
...
Overall the refactoring left the observable system behaviour unchanged, but the Event Router was now part of the Transitional Architecture, having been inserted into the message processing pipeline.
...
Bill Codding, Chris Ford, James Emmott, Kief Morris, Mark Taylor, Meaghan Waters, Peter Gillard-Moss, Rebecca Parsons, and Simon Brunning discussed drafts of these patterns on our internal mailing lists. James Emmott suggested “displacing” in the title.

The photo of the new and old Manchester trams used in the card illustration is by [Picasa](https://commons.wikimedia.org/wiki/File:T68_and_M5000.JPG), cropped and color treated.

Significant Revisions

_05 March 2024:_ Event Interception

_07 July 2022:_ Revert to Source Published

_28 March 2022:_ Transitional Architecture Initial Publication

_20 January 2022:_ Divert the Flow

_19 January 2022:_ Critical Aggregator

_12 January 2022:_ Legacy Mimic

_27 July 2021:_ Feature Parity

_21 July 2021:_ Extract Product Lines

_20 July 2021:_ Patterns of Legacy Displacement Published
